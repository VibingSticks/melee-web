...
#ifdef TARGET_PC
/* One block, so the offsets the casts rely on are real here too (see toy.h). */
struct ToyStateBlock Toy_state_block;
ASSERT_OFFSET(struct ToyStateBlock, state, 0x194);
ASSERT_OFFSET(struct ToyStateBlock, anim, 0x3F0);
#define _Toy_804A26B8 (Toy_state_block.head)
#define _Toy_devtext_buf_804A26C4 (Toy_state_block.devtext1)
#define _Toy_devtext_buf_804A2750 (Toy_state_block.devtext2)
#else
