...
/* Signed variant, for the SIS stream's s16 offsets. */
static inline s16 sisReadS16(const void* p)
{
    return (s16) sisReadU16(p);
}

/* 32-bit variant. Used for the saved-state stack, whose push side writes the
 * bytes high-first with plain arithmetic and so is big-endian on every host,
 * not just the GameCube. */
static inline u32 sisReadU32(const void* p)
{
#ifdef TARGET_PC
    const u8* b = (const u8*) p;
...
s32 HSD_SisLib_803A7F0C(HSD_Text* text, s32 flags)
{
    ...
    while (pos >= 0) {
        ...
        case 1:
            ...
            if (target_type == 1) {
                text->x78.x =
                    (f32) sisReadS16(text->string_buffer + pos) / 256.0F;
                text->x78.y =
                    (f32) sisReadS16(text->string_buffer + pos + 2) / 256.0F;
        ...
        case 3:
            ...
            if (target_type == 3) {
                text->x80.x =
                    (f32) sisReadU16(text->string_buffer + pos) / 256.0F;
                text->x80.y =
                    (f32) sisReadU16(text->string_buffer + pos + 2) / 256.0F;
                if (flag_hi == entry_flags) {
                    remove_size = 5;
                }
                goto done;
        ...
        case 5:
            pos -= 4;
            if (target_type == 5) {
                result = (s32) sisReadU32(text->string_buffer + pos);
                if (flag_hi == entry_flags) {
                    remove_size = 5;
                }
                goto done;
...
loop_3:
    ...
    case 14:
        ...
        text->x80.x = (f32) sisReadU16((u8*) cursor + 1) / 256.0F;
    ...
    case 10:
        if ((text->alloc_data == NULL) || (kern_enabled == 0)) {
            ...
            text->x78.x = (f32) sisReadS16((u8*) cursor + 1) / 256.0F;
        }
        cursor = (u8*) cursor + 4;
        goto block_33;
    case 11:
                ...
                if ((sis_cursor == text->x60) && text->x94) {
                    ...
                    switch (*sis_cursor) {
                        ...
                        case 7:
                            ...
                            x_origin = (f32) sisReadS16(sis_cursor + 1);
                            ...
                            y_offset = sisReadS16(sis_cursor + 3);
                            sis_cursor += 4;
                        ...
                        case 10:
                            if (((u32) text->alloc_data == 0U) || (saved_kerning == 0)) {
                                ...
                                text->x78.x = (f32) sisReadS16(sis_cursor + 1) / 256.0F;
                                text->x78.y = (f32) sisReadS16(sis_cursor + 3) / 256.0F;
                            }
                            sis_cursor += 4;
                            break;
                        case 11:
